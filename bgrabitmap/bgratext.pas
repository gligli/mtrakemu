...
uses
  Classes, Types, SysUtils, Graphics, BGRABitmapTypes, InterfaceBase;

type

  { TLCLFontRenderer }

  TLCLFontRenderer = class(TBGRACustomFontRenderer)
  protected
...
uses Math, BGRABlend, LCLPlatformDef;
...
procedure TLCLFontRenderer.UpdateFont;
begin
...
function TLCLFontRenderer.GetFontPixelMetric: TFontPixelMetric;
var fxFont: TFont;
begin
  UpdateFont;
...
procedure TLCLFontRenderer.TextOutAngle(ADest: TBGRACustomBitmap; x, y: single; orientation: integer;
...
procedure TLCLFontRenderer.TextOut(ADest: TBGRACustomBitmap; x, y: single; s: string;
...
begin
  ...
  if Assigned(BGRATextOutImproveReadabilityProc) and (FontQuality in[fqFineAntialiasing,fqFineClearTypeBGR,fqFineClearTypeRGB]) and (FFont.Orientation mod 3600 = 0) then
    BGRATextOutImproveReadabilityProc(ADest,FFont,x,y,s,BGRAPixelTransparent,texture,align,
...
procedure TLCLFontRenderer.TextOut(ADest: TBGRACustomBitmap; x, y: single; s: string; c: TBGRAPixel;
  align: TAlignment);
begin
  UpdateFont;

  if Assigned(BGRATextOutImproveReadabilityProc) and (FontQuality in[fqFineAntialiasing,fqFineClearTypeBGR,fqFineClearTypeRGB]) and (FFont.Orientation mod 3600 = 0) then
    BGRATextOutImproveReadabilityProc(ADest,FFont,x,y,s,c,nil,align,
    FontQuality in[fqFineClearTypeRGB,fqFineClearTypeBGR], FontQuality = fqFineClearTypeRGB) else

    BGRAText.BGRATextOut(ADest,FFont,FontQuality,x,y,s,c,nil,align);
end;

procedure TLCLFontRenderer.TextRect(ADest: TBGRACustomBitmap; ARect: TRect; x, y: integer; s: string;
...
function TLCLFontRenderer.TextSize(s: string): TSize;
begin
  UpdateFont;
...
constructor TLCLFontRenderer.Create;
begin
  FFont := TFont.Create;
end;

destructor TLCLFontRenderer.Destroy;
begin
  FFont.Free;
  inherited Destroy;
end;
