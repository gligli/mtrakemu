...
uses
  Types, Classes, SysUtils, Graphics, BGRABitmapTypes, BGRATypewriter, BGRATransform, BGRACanvas2D, LazUTF8;
...
type
  ...
  public
    constructor Create;
    constructor Create(AVectorize: boolean);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (bestIndex = -1) or (FDirectoryContent[i].FontStyle = FStyle) or
              (((fsBold in bestStyle) xor (fsBold in FStyle)) and
             ((fsItalic in bestStyle) xor (fsItalic in FStyle))) then
        begin
          bestIndex := i;
          bestStyle := FDirectoryContent[i].FontStyle;
          if bestStyle = FStyle then break;
        end;
      end;
    end;
    if bestIndex <> -1 then
...
constructor TBGRAVectorizedFont.Create(AVectorize: boolean);
begin
  inherited Create;
  Init(AVectorize);
end;
