...
type
  ...
  public
    ...
    function GetPixel(x, y: int64): TBGRAPixel; override;
...
function TBGRADefaultBitmap.GetPixel(x, y: int64): TBGRAPixel;
begin
  if (x < 0) or (x >= Width) or (y < 0) or (y >= Height) then //it is possible to read pixels outside of the cliprect
    Result := BGRAPixelTransparent
  else
...
procedure TBGRADefaultBitmap.FillShapeAntialias(shape: TBGRACustomFillInfo;
  c: TBGRAPixel);
begin
  BGRAPolygon.FillShapeAntialias(self, shape, c, FEraseMode, nil, FillMode = fmWinding);
end;

procedure TBGRADefaultBitmap.FillShapeAntialias(shape: TBGRACustomFillInfo;
  texture: IBGRAScanner);
begin
  BGRAPolygon.FillShapeAntialiasWithTexture(self, shape, texture, FillMode = fmWinding);
end;
...
begin
  FontRenderer.TextOutAngle(self,x,y,orientation,s,c,align);
end;

procedure TBGRADefaultBitmap.TextOutAngle(x, y: single; orientation: integer;
  s: string; texture: IBGRAScanner; align: TAlignment);
begin
  FontRenderer.TextOutAngle(self,x,y,orientation,s,texture,align);
end;

procedure TBGRADefaultBitmap.TextOut(x, y: single; s: string;
  texture: IBGRAScanner; align: TAlignment);
begin
  FontRenderer.TextOut(self,x,y,s,texture,align);
end;

procedure TBGRADefaultBitmap.TextOut(x, y: single; s: string;
  c: TBGRAPixel; align: TAlignment);
begin
  FontRenderer.TextOut(self,x,y,s,c,align);
end;
